...
class AIService {
     ...
     */
    async searchEmployeesSemantically(query) {
        await this.ensureVectorExtension();
        const queryEmbedding = await this.generateEmbedding(query);
        const formattedVector = `[${queryEmbedding.join(",")}]`;
        const matches = await prisma_1.default.$queryRawUnsafe(`SELECT e.id, e."userId", e.content, 1 - (e.embedding <=> $1::vector) AS similarity,
              u."firstName", u."lastName", u.email, u.role, u."isActive"
       FROM "EmployeeEmbedding" e
       JOIN "User" u ON e."userId" = u.id
       ORDER BY e.embedding <=> $1::vector ASC
       LIMIT 10`, formattedVector);
        return matches;
    }
    /**
     * Answer attendance-related natural language questions using AI tool/function calling
     */
    async askAttendanceQuestion(question) {
        const { client, modelName } = getAIClient();
